  ...
  run: async (client, message, args) => {
    ...
    if (!Member)
      return message.channel.send(
        `Kérlek adj megy egy embert akit meg akarsz hackelni! `
      );

    if (Member.user.id === message.author.id)
      return message.channel.send(`Nem tudod magad meg hackelni Lmao!`);

    let embed = new MessageEmbed()
      .setColor(Color)
      .setTitle(`Hack Status: Completed`)
      .setDescription(
        `Név: ${Member.user.username} | ID: ${
          Member.user.id
        }`
      )
      .setFooter(`Jk |Komolyan azt hitted ,hogy ÉN valakit meg hackelek? `)
      .setTimestamp();
